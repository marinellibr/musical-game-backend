...
import { getSpotifyAlbumTracks, spotifySearch, SpotifyTrack } from "../../integrations/spotify/spotifyClient";
...
router.get("/search", async (req, res) => {
  ...
  try {
    ...
    return res.json({ query: q, items: (result.tracks?.items || []).map(publicTrack) });
  } catch {
...
router.get("/albums/:albumId/tracks", async (req, res) => {
  ...
  try {
    ...
    return res.json({ albumId: req.params.albumId, items: (result.items || []).map(publicTrack) });
  } catch {
    return res.status(503).json({ error: { code: "SPOTIFY_UNAVAILABLE", message: "Spotify album is unavailable" } });
  }
});

function publicTrack(track: SpotifyTrack) {
  return { trackId: track.id, trackUri: track.uri, title: track.name, artist: track.artists?.map((artist) => artist.name).filter(Boolean).join(", "), album: track.album?.name, albumId: track.album?.id, image: track.album?.images?.[0]?.url };
}
